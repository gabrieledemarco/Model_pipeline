#!/usr/bin/env python3
"""
create_asia_sweep_mss_chart_examples.py
==========================================
Grafici di esempio per la strategia Asia Range Sweep + MSS + FVG/OB
(create_asia_sweep_mss_report.py): candlestick 1H con marker per range
Asia, sweep, market structure shift, zona FVG/OB, entry, target, stop,
uscita. Seleziona alcuni trade vincenti e perdenti (RR=2:1) per illustrare
la meccanica della strategia.
"""
    ...
    for i in range(max(leg_start, 1), min(leg_end, N1H - 2)):
        ...
        if direction == 1 and HI[i - 1] < LO[i + 1]:
            ...
            for j in range(start_j, min(i + 2 + FVG_OB_MAX_AGE, cutoff_i)):
                if CL[j] < fbot: break
                if LO[j] <= ftop:
                    trig = dict(j=j, price=min(CL[j], ftop), kind="FVG",
                                zone_lo=fbot, zone_hi=ftop, form_i=i)
        ...
        if direction == -1 and LO[i - 1] > HI[i + 1]:
            ...
            for j in range(start_j, min(i + 2 + FVG_OB_MAX_AGE, cutoff_i)):
                if CL[j] > ftop: break
                if HI[j] >= fbot:
                    trig = dict(j=j, price=max(CL[j], fbot), kind="FVG",
                                zone_lo=fbot, zone_hi=ftop, form_i=i)
                    if best is None or trig["j"] < best["j"]: best = trig
                    break
    return best
        ...
        if direction == 1 and OP[i] > CL[i]:
            ...
            for j in range(start_j, min(i + FVG_OB_MAX_AGE, cutoff_i)):
                ...
                if LO[j] <= ob_hi and HI[j] >= ob_lo:
                    trig = dict(j=j, price=max(CL[j], ob_lo), kind="OB",
                                zone_lo=ob_lo, zone_hi=ob_hi, form_i=i)
                    if best is None or trig["j"] < best["j"]: best = trig
                    break
        if direction == -1 and OP[i] < CL[i]:
            ob_hi, ob_lo = HI[i], LO[i]
            ...
            for j in range(start_j, min(i + FVG_OB_MAX_AGE, cutoff_i)):
                ...
                if HI[j] >= ob_lo and LO[j] <= ob_hi:
                    trig = dict(j=j, price=min(CL[j], ob_hi), kind="OB",
                                zone_lo=ob_lo, zone_hi=ob_hi, form_i=i)
                    if best is None or trig["j"] < best["j"]: best = trig
                    break
    return best
...
for day in all_days:
    ...
    for j in range(sweep_i + 1, min(sweep_i + 1 + MSS_MAX_BARS, N1H)):
        if direction == 1 and CL[j] > ref_level: mss_i = j; break
        if direction == -1 and CL[j] < ref_level: mss_i = j; break
    if mss_i < 0: continue
...
# ── Backtest a single event, RR fixed ──────────────────────────────────────
def simulate(ev):
...
# pick 2 wins + 2 losses spread across different years, prefer OB and FVG mix
...
fig.suptitle("Asia Range Sweep + MSS + FVG/OB v2 (rejection-filtered sweep) — Trade Examples (RR=2:1)",
...
for ax, ev in zip(axes.flat, examples):
    ...
    plot_start = max(min(ev["sweep_i"] - 10, asia_start_i - 2), 0)
    plot_end = min(ev["sim"]["exit_i"] + 6, N1H - 1)
    ...
    ax.add_patch(Rectangle((mdates.date2num(form_t), zone["zone_lo"]),
                            mdates.date2num(entry_t) - mdates.date2num(form_t) + 0.02,
                            zone["zone_hi"] - zone["zone_lo"],
                            facecolor=zcolor, alpha=0.25, edgecolor=zcolor, linewidth=1, zorder=1,
    ...
    ax.set_title(f"{ev['day'].date()}  {dirlabel}  |  exit={ev['sim']['out'].upper()}  "
...
out_path = Path("reports/asia_sweep_mss_v2_examples.png")
