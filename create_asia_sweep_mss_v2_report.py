#!/usr/bin/env python3
"""
create_asia_sweep_mss_v2_report.py
=====================================
v2 — sweep più selettivo: non basta che il prezzo attraversi l'estremo e
chiuda di nuovo dentro il range, la candela di sweep deve mostrare un vero
RIGETTO (coda più lunga del corpo, tipo pin-bar) per essere considerata uno
sweep valido. Richiesta esplicita dell'utente dopo aver visto gli esempi
v1: "non tutti gli sweep che ho visto sono validi... una o più candele deve
attraversare l'estremo del range ed essere rigettato, cerca candele con
code più lunghe del body".

Tutto il resto della sequenza è invariato rispetto a v1
...
Sequenza (tutto su barre 1H, causale):
  ...
  2. SWEEP (v2 — con filtro di rigetto): prima barra (07:00 UTC -> fine
     giornata) che spazza un estremo, chiude di nuovo dentro il range, E
     ha una coda di rigetto più lunga del corpo:
       bullish sweep: low < asia_lo  AND  close > asia_lo
                       AND (min(open,close)-low) > |close-open|   -> LONG
       bearish sweep: high > asia_hi AND  close < asia_hi
                       AND (high-max(open,close)) > |close-open|  -> SHORT
...
w(SEP)
w("Asia Range Sweep + MSS + FVG/OB Entry v2 (rejection-filtered sweep) — Validation Pipeline")
w(SEP)
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/asia_sweep_mss_v2.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# Asia Range Sweep + MSS + FVG/OB Entry v2 (rejection-filtered sweep) — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
