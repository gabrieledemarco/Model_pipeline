#!/usr/bin/env python3
"""
create_asia_sweep_mss_v3_report.py
=====================================
v3 — filtro premium/discount: il FVG o Order Block è accettato come entry
SOLO se si trova nella metà "a sconto" (discount, per i LONG) o "a premio"
(premium, per gli SHORT) del trading range definito dallo sweep — richiesta
esplicita dell'utente: "il fvg o order block vanno ricercati sempre e solo
in zone di discount rispetto al trading range in cui si trova la struttura
di mercato". Interpretazione standard ICT (buy discount / sell premium,
simmetrica per direzione — l'alternativa "discount per entrambe le
direzioni" non avrebbe senso geometrico dato che discount/premium sono
definiti relativamente allo stesso equilibrium):

  Trading range della gamba di sweep→MSS:
    LONG  : leg_lo = low della barra di sweep, leg_hi = ref_level (l'ultimo
            pivot high confermato, quello la cui rottura conferma l'MSS)
    SHORT : leg_hi = high della barra di sweep, leg_lo = ref_level (pivot low)
    equilibrium = (leg_lo + leg_hi) / 2       (il 50% ICT del range)

  Filtro di accettazione sull'entry (prezzo di fill FVG/OB):
    LONG  : entry_price < equilibrium   (discount — metà inferiore del range)
    SHORT : entry_price > equilibrium   (premium — metà superiore del range)

Tutto il resto della sequenza è invariato rispetto a v2
(create_asia_sweep_mss_v2_report.py, che ha già il filtro di rigetto sulla
candela di sweep — coda più lunga del corpo):

Sequenza (tutto su barre M15, causale):
  1. RANGE ASIA: sessione 00:00-07:00 UTC.
  2. SWEEP (con filtro di rigetto, v2): candela che spazza un estremo,
     chiude di nuovo dentro il range, E ha coda di rigetto > corpo.
  3. MARKET STRUCTURE SHIFT (MSS): rottura in chiusura dell'ultimo pivot
     fractal confermato (left=right=3) nella direzione del sweep.
  4. ENTRY su FVG o Order Block, filtrato per premium/discount (v3, vedi
     sopra) rispetto al trading range sweep→ref_level.
...
w(SEP)
w("Asia Range Sweep + MSS + FVG/OB Entry v3 (rejection sweep + premium/discount + M15) — Validation Pipeline")
w(SEP)
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/asia_sweep_mss_v3.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# Asia Range Sweep + MSS + FVG/OB Entry v3 (rejection sweep + premium/discount + M15) — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
