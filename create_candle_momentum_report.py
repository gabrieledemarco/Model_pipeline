#!/usr/bin/env python3
"""
create_candle_momentum_report.py
===================================
Nuova strategia proposta dall'utente: momentum di brevissimo periodo su
barre 1 minuto.

  LONG  dopo una candela VERDE (close > open)
  SHORT dopo una candela ROSSA (close < open)
...
w(SEP)
w("Candle Momentum (green/red, volume filter, 1-15min hold) — Validation Pipeline")
w(SEP)
...
def run_bt(hold_min, volume_filter, slippage_pct=0.0):
    ...
    long_sig = green & (vol_hi if volume_filter else True)
    short_sig = red & (vol_hi if volume_filter else True)
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/candle_momentum.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# Candle Momentum (green/red, volume filter, 1-15min hold) — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
