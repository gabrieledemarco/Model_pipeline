#!/usr/bin/env python3
"""
create_fvg_sniper_grade_sizing_report.py
============================================
Interviene sul POSITION SIZING invece che su un altro filtro di ingresso
(per evitare di impilare soglie ottimizzate, rischio di overfitting
segnalato dall'utente). Nota tecnica preliminare: un ridimensionamento
UNIFORME (es. rischiare 0.5% invece di 1% su ogni trade) non cambia
Sharpe/MC p(profit)/DSR — sono rapporti scala-invarianti, guadagni e
perdite si riducono nella stessa proporzione. Serve un sizing NON
uniforme per avere un effetto reale sulla qualità statistica, non solo
sulla magnitudine in dollari.

Intervento (formula fissa, nessun grid/soglia da ottimizzare — il
segnale di base resta MIN_GRADE=4.0, la soglia hard originale dello
script Pine, invariata): il rischio per trade è proporzionale al grade
stesso del gap (già calcolato dall'indicatore su scala 0-10):

    risk_pct(trade) = RISK_PCT_BASE * (grade / 10.0)

Un gap grade=4 (appena sopra la soglia hard) rischia il 40% del budget
base; un gap grade=10 rischia il 100%. Usa un'informazione già nota al
momento dell'entry (nessun lookahead), pesa in continuo invece di
scegliere in blocco quali trade tenere — i trade a basso grade (quelli
che l'autopsy ha mostrato dominare le perdite peggiori del 2025-2026)
restano nel campione ma pesano meno sul risultato aggregato.

Motore a posizioni concorrenti identico a v2, ma ora il cap sul rischio
aggregato (MAX_TOTAL_RISK_PCT) somma i rischi VARIABILI delle posizioni
aperte invece di un rischio fisso identico per tutte.
"""
...
MIN_GAP_ATR = 0.25
MIN_BODY_RATIO = 0.45
MIN_GRADE = 4.0                  # soglia hard originale, INVARIATA (nessuna ri-ottimizzazione)
HTF_EMA_LEN = 50
SL_BUF_ATR = 0.20
MIN_RISK_ATR = 0.40
MAX_RISK_ATR = 4.00
...
w(SEP)
w("FVG Sniper — IFVG Flip Only — Position sizing proporzionale al grade")
w(SEP)
w("\nrisk_pct(trade) = 1% x (grade/10) — formula fissa, nessun grid. Segnale")
w("di base invariato (MIN_GRADE=4.0, soglia hard originale dello script).")
...
def build_candidates(signals, HI, LO, OP, CL, ATR, N):
    ...
    for s in sorted(signals, key=lambda s: s["idx"]):
        ...
        if risk <= 0: continue
        risk_pct_trade = RISK_PCT_BASE * (s["grade"] / GRADE_NORM)
        if risk < risk_pct_trade * ep / MAX_LEV: continue
...
def run_bt_portfolio(evs, extra_slippage_pct=0.0, max_total_risk_pct=MAX_TOTAL_RISK_PCT):
    ...
    def close_position(pos, exit_price, reason):
        ...
        risk_budget_dollar = INIT_CAP * pos["risk_pct"]
    ...
    for i in range(min_i, max_i + 1):
        ...
        for ev in by_entry.get(i, []):
            ev_risk_dollar = INIT_CAP * ev["risk_pct"]
            if total_open_risk + ev_risk_dollar > max_total_risk_dollar + 1e-9:
                continue
            open_positions.append(dict(entry_i=ev["entry_i"], d=ev["d"], ep=ev["ep"], sl=ev["sl"],
                                        target=ev["target"], risk=ev["risk"], risk_pct=ev["risk_pct"],
                                        deadline_i=ev["entry_i"] + ev["hold"]))
            total_open_risk += ev_risk_dollar
    for pos in open_positions:
...
print("[ENGINE] Detecting FVGs + IFVG Flip signals (MIN_GRADE=4.0, sizing grade-proporzionale) …")
...
avg_risk_pct = np.mean([e["risk_pct"] for e in evs]) * 100
print(f"  {len(signals):,} segnali -> {len(evs):,} trade (holdout: {len(holdout_evs):,})  "
      f"risk_pct medio={avg_risk_pct:.2f}%")
...
w(f"\n{SEP}")
w("CONFRONTO — sizing grade-proporzionale vs filtro binario MIN_GRADE=5.0 vs baseline")
w(SEP)
w(f"\n                              n      Ret%      WR    MC pp(full)   ret_hold%   MC pp(hold)")
w(f"  Sizing grade-prop. (qui) {n:>6}   {res['ret']:>+6.1f}%   {res['wr']:>5.1%}      {mc['p_profit']:>6.3f}      "
  f"{hres['ret']:>+7.1f}%       {hmc['p_profit']:>6.3f}")
w(f"  Filtro MIN_GRADE=5.0       1725   +225.3%   33.9%      1.000        +15.2%        0.644")
w(f"  Baseline MIN_GRADE=4.0     1995   +200.9%   33.2%      0.999         -9.8%        0.414")

w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/fvg_sniper_grade_sizing.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# FVG Sniper — Position sizing proporzionale al grade\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
