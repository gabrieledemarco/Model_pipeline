#!/usr/bin/env python3
"""
create_fvg_sniper_report.py
==============================
Sottopone "FVG Sniper [JOAT]" (indicatore Pine v6 condiviso dall'utente)
alla pipeline di validazione della sessione. Replica fedelmente la logica
dello script: gap grading (size/displacement/volume, 0-10), state machine
per gap (mitigated/inverted/reacted), 2 sorgenti di segnale (Rejection =
prezzo rientra in un gap vivo e rigetta con chiusura direzionale; IFVG
Flip = un gap viene attraversato per intero e inverte ruolo, si tratta la
continuazione), filtro di bias HTF (4H EMA50), stop "Gap-Protected"
(oltre il bordo del gap trigger + buffer ATR, con floor/cap ATR sul
rischio), target singolo a TP3=3R (TP1/TP2 nello script originale sono
solo livelli visivi, non uscite parziali reali — la logica di uscita usa
solo tp3R come target pieno).

Deviazione dalla lettura letterale dello script (dichiarata): Pine calcola
`entv = close` sulla barra stessa del segnale (barstate.isconfirmed la
rende "chiusa" nel momento in cui il codice gira) — per coerenza con lo
standard causale di questa sessione (mai un fill esattamente alla
chiusura della barra di segnale) si usa l'apertura della barra
SUCCESSIVA come entry, identico a tutte le altre strategie validate.

Griglia DSR: le 3 modalità di segnale esposte dall'autore stesso
(Rejection Only / IFVG Flip Only / Rejection + IFVG, quest'ultima il
default). Frizioni Bybit reali, RISK_PCT 1% non-compounding, MAX_LEV=10,
holdout 2025-2026 genuino, Monte Carlo i.i.d.+block, slippage-stress.
"""
...
# ── Parametri identici ai default dello script Pine ──────────────────────
ATR_LEN = 14
MIN_GAP_ATR = 0.25
MIN_BODY_RATIO = 0.45
MIN_GRADE = 4.0
...
w(SEP)
w('FVG Sniper [JOAT] — Validation Pipeline')
w(SEP)
w("\nReplica fedele dell'indicatore Pine condiviso: gap grading, state")
w("machine mitigated/inverted/reacted, segnali Rejection + IFVG Flip,")
w("filtro bias 4H EMA50, stop gap-protected, target singolo 3R.")
...
# ── HTF bias (4H EMA50, causale — solo barre 4H già chiuse) ───────────────
...
def map_htf_bias(idx_ltf):
    """Bias dell'ultima barra 4H CHIUSA prima (o al più uguale) dell'apertura
    della barra LTF corrente — replica lookahead_off di request.security."""
...
# ── State machine gap + generazione segnali (identico allo script) ────────
def run_engine(fvg_births, HI, LO, OP, CL, N, htf_bias, mode):
    ...
    active = []   # list of dict gap state
    signals = []  # dict(idx, dir, mode_tag, top, bot)
...
def build_trades(signals, HI, LO, OP, CL, ATR, N):
    """Un trade alla volta (come tradeActive nello script), entry = apertura
    barra successiva al segnale (deviazione dichiarata, vedi docstring)."""
...
def run_bt(evs, HI, LO, CL, N, extra_slippage_pct=0.0):
    ...
    n_tp = n_sl = n_time = 0
    for ev in evs:
        d, ep, sl, target, risk, entry_i, hold = (ev["d"], ev["ep"], ev["sl"], ev["target"],
                                                    ev["risk"], ev["entry_i"], ev["hold"])
...
for tf in TIMEFRAMES:
    ...
    for mode in SIGNAL_MODES:
        ...
        evs = build_trades(signals, HI, LO, OP, CL, ATR, N)
        holdout_evs = [e for e in evs if d.index[e["entry_i"]] >= CUTOFF]

        w(f"\n{SEP}")
        w(f"[{tf}] Mode = {mode}")
        w(SEP)
        w(f"\n  {len(signals):,} segnali -> {len(evs):,} trade sequenziali "
          f"(holdout 2025-2026: {len(holdout_evs):,})")

        res = run_bt(evs, HI, LO, CL, N)
        n = res["n"]
        ...
        for yr in sorted(year_evs):
            ...
            if len(yevs) < 5:
                continue
            yres = run_bt(yevs, HI, LO, CL, N)
            w(f"    {yr:>6}  {yres['n']:>6}  {yres['ret']:>+7.1f}%  {yres['wr']:>5.1%}")

        hres = run_bt(holdout_evs, HI, LO, CL, N)
    ...
    best_evs = build_trades(best_signals, HI, LO, OP, CL, ATR, N)
    best_holdout_evs = [e for e in best_evs if d.index[e["entry_i"]] >= CUTOFF]
    for bps in SLIPPAGE_STRESS_BPS:
        extra = bps / 10_000.0
        for scope_name, evs_s in [("full-sample", best_evs), ("holdout", best_holdout_evs)]:
            r = run_bt(evs_s, HI, LO, CL, N, extra_slippage_pct=extra)
            m = mc_summary(r["net_pnls"])
            w(f"  {bps:>7}bps  {scope_name:>10}  {r['n']:>6}  {r['ret']:>+7.1f}%  "
              f"{r['wr']:>5.1%}  {m['p_profit']:>6.3f}  {m['p_ruin']:>6.3f}")

w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/fvg_sniper_validation.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# FVG Sniper [JOAT] — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
