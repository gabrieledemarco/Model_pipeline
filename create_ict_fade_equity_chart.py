#!/usr/bin/env python3
"""
create_ict_fade_equity_chart.py
==================================
Grafico evoluzione capitale: strategia validata (fade dei trigger ICT
standalone, RR=3.0, `ict_fade_standalone.md`) vs buy & hold BTCUSDT,
stesso capitale iniziale, stesso periodo (2020-2026), con l'holdout
genuino 2025-2026 evidenziato.
"""
...
def run_bt_curve(evs, rr):
    cap = INIT_CAP
    curve_t, curve_cap = [IDX[0]], [INIT_CAP]
    for ev in evs:
        ...
        slip = SLIPPAGE_BASE
        fill_ep = ep * (1 + d * slip)
        fill_xp = exit_price * (1 - d * slip)
        pnl = units * (fill_xp - fill_ep) * d - FEE_TAKER * units * fill_ep - FEE_TAKER * units * fill_xp
        cap += pnl
        curve_t.append(IDX[exit_j]); curve_cap.append(cap)
    curve_t.append(IDX[-1]); curve_cap.append(cap)
    return pd.Series(curve_cap, index=pd.DatetimeIndex(curve_t))


equity = run_bt_curve(ALL_EVENTS, RR)
print(f"[EQUITY] final cap = {equity.iloc[-1]:,.0f}  ({(equity.iloc[-1]/INIT_CAP-1)*100:+.1f}%)")

bh = pd.Series(CL, index=IDX)
bh_norm = bh / bh.iloc[0] * INIT_CAP

fig, ax = plt.subplots(1, 1, figsize=(18, 9), facecolor=_BG)
ax.set_facecolor(_CARD)

ax.axvspan(CUTOFF, IDX[-1], color=_YEL, alpha=0.08, zorder=0, label="Holdout genuino 2025-2026")

ax.plot(bh_norm.index, bh_norm.values, color=_BLU, linewidth=1.4, label="Buy & Hold BTCUSDT", zorder=2)
eq_step = equity.reindex(bh_norm.index, method="ffill").bfill()
ax.plot(eq_step.index, eq_step.values, color=_GRN, linewidth=1.8,
        label="Strategia: Fade ICT standalone (RR=3.0)", zorder=3)

ax.axhline(INIT_CAP, color="#555", linestyle=":", linewidth=1, zorder=1)
ax.axvline(CUTOFF, color=_YEL, linestyle="--", linewidth=1, alpha=0.7, zorder=1)

ax.set_yscale("log")
ax.set_title("Evoluzione capitale: Strategia (Fade ICT standalone, RR=3.0) vs Buy & Hold BTCUSDT — 2020-2026",
              color=_TEXT, fontsize=14)
ax.set_ylabel("Capitale (USD, scala log)", color=_TEXT)
ax.tick_params(colors=_TEXT)
for spine in ax.spines.values(): spine.set_color("#333")
ax.legend(loc="upper left", fontsize=11, facecolor=_CARD, edgecolor="#333", labelcolor=_TEXT)
ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m"))
ax.grid(True, color="#222", linewidth=0.5, zorder=0)

final_strat = equity.iloc[-1]
final_bh = bh_norm.iloc[-1]
ax.annotate(f"{final_strat:,.0f} ({(final_strat/INIT_CAP-1)*100:+.0f}%)", xy=(equity.index[-1], final_strat),
            xytext=(10, 5), textcoords="offset points", color=_GRN, fontsize=10, fontweight="bold")
ax.annotate(f"{final_bh:,.0f} ({(final_bh/INIT_CAP-1)*100:+.0f}%)", xy=(bh_norm.index[-1], final_bh),
            xytext=(10, -15), textcoords="offset points", color=_BLU, fontsize=10, fontweight="bold")

plt.tight_layout()
out = Path("reports/ict_fade_equity_vs_bh.png")
plt.savefig(out, dpi=120, facecolor=_BG)
print(f"[SAVED] {out}   (total runtime {time.time()-t0:.0f}s)")
