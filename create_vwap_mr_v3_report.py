#!/usr/bin/env python3
"""
create_vwap_mr_v3_report.py
===============================
v3 — come sfruttare l'edge direzionale trovato in v1 (win rate 61-66%
stabile OGNI SINGOLO ANNO 2020-2026, il più stabile di tutta la sessione)
senza farlo distruggere dalla costruzione del trade:
...
Leva mai esplorata: il moltiplicatore ATR dello stop era fissato a 2.0 A
PRIORI in v1 (mai scansionato — l'IS-scan variava solo Z_ENTRY e filtro
regime). v3 aggiunge SL_ATR_MULT come terzo asse dell'IS-scan causale per
finestra WFO, per lasciare che la selezione (su dati IS, mai OOS) trovi il
punto di equilibrio tra "troppo largo" (v1) e "troppo stretto" (v2) invece
di indovinarlo.

Tutto il resto dell'architettura è invariato rispetto a v1 — stessa
architettura a 3 layer di S07 OU Mean Reversion (l'unica strategia
validata "storicamente" in questa sessione), con l'ingrediente
"equilibrio" sostituito: VWAP di sessione (ancorato al giorno UTC) al
posto del processo Ornstein-Uhlenbeck.
...
/ step 2m) su una griglia Z_ENTRY×filtro×SL_ATR_MULT (3×2×6=36 combo),
...
SL_ATR_MULT_GRID = [0.5, 0.75, 1.0, 1.25, 1.5, 2.0]
...
w(SEP)
w("VWAP Mean-Reversion v3 — SL ATR-multiple IS-scan — Validation Pipeline")
w(SEP)
...
def build_events(idx_range, z_entry, filter_variant, hmm_state, sl_atr_mult):
    evs = []
    for i in idx_range:
        ...
        if filter_variant == "SIDEWAYS_ONLY":
            if hmm_state[i] != 1:   # 1 = sideways (see HMM_FEATURE_NAMES semantics)
                continue
        evs.append(dict(i=i, d=d, ep=CL[i], sl_atr_mult=sl_atr_mult))
    return evs
...
w(f"  SL_ATR_MULT più scelto: {sel_df['sl_mult'].value_counts().to_dict() if not sel_df.empty else {}}")
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/vwap_mr_v3.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# VWAP Mean-Reversion v3 — SL ATR-multiple IS-scan — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
