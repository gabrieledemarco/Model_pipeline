#!/usr/bin/env python3
"""
create_vwap_mr_v4_report.py
===============================
v4 — terzo tentativo di sfruttare l'edge direzionale trovato in v1 (win
rate 61-66% stabile OGNI SINGOLO ANNO 2020-2026, il più stabile di tutta
la sessione):

  v1: stop fisso a 2×ATR (largo)              -> WR reale preservato (62.9%)
      ma R:R sfavorevole (stop ~1.83× il target) -> ret netto -182.7%
  v2: stop proporzionale al target (RR fisso) -> R:R sistemato ma WR
      CROLLA (fino al 34.6% a RR=2) per rumore sullo stop stretto
  v3: scan causale del moltiplicatore ATR dello stop (0.5-2.0) -> l'IS-scan
      sceglie 2.0 (il massimo testato) nella maggioranza delle finestre,
      confermando che lo stop largo è ciò che serve, ma il risultato OOS
      aggregato è PEGGIORE di v1 — overfitting da troppi gradi di libertà
      nella selezione causale su finestre IS piccole.

v4 cambia completamente l'asse di leva: invece di continuare a tarare UNA
soglia di prezzo come uscita primaria (che in ogni sua forma, larga o
stretta, soffre dell'asimmetria di first-passage-time), si applica la
ricetta che ha funzionato per l'unica ALTRA strategia validata in questa
sessione oltre a S07 — la ML RandomForest 8h:

  USCITA A TEMPO FISSO come meccanismo primario (non il tocco del VWAP),
  con un solo stop di sicurezza LARGO E FISSO (non scansionato, per
  evitare la trappola di overfitting di v3) per la sola protezione da
  eventi estremi.

  hold_hours ∈ {2, 4, 8} è scansionato causalmente insieme a Z_ENTRY e
  filtro regime (stesso principio walk-forward di v1/v3), ma SL_ATR_MULT
  è fissato a 3.0 (largo, safety-net, non un asse di ricerca).

Il resto dell'architettura è invariato — stessa base a 3 layer di S07 OU
Mean Reversion, con l'ingrediente "equilibrio" sostituito: VWAP di sessione
(ancorato al giorno UTC) al posto del processo Ornstein-Uhlenbeck.
...
Layer 3 — Sizing/Exit (v4):
  Uscita primaria: A TEMPO FISSO, hold_hours ∈ {2,4,8} (scansionato per
        finestra), non più il tocco del VWAP — fedele a cosa il segnale
        realmente misura (uno stretch statistico che tende a rientrare,
        non un livello di prezzo preciso da colpire)
  SL  : safety-net fisso a 3.0×ATR_1H (largo, non scansionato — protezione
        da eventi estremi, non pensato per attivarsi spesso)
  Cap aggiuntivo: comunque non oltre la fine della giornata UTC (il VWAP
        si resetta il giorno dopo, tenere la posizione oltre non ha senso)

Selezione parametri: IS-scan causale per ogni finestra WFO (6m IS / 2m OOS
/ step 2m) su una griglia Z_ENTRY×filtro×hold_hours (3×2×3=18 combo),
selezione per Sharpe IS — stesso principio del TP/SL IS-scan di S07. Nessun
DSR richiesto sul risultato finale: la selezione avviene causalmente per
...
SAFETY_SL_ATR_MULT = 3.0   # fisso, largo, non scansionato (evita l'overfitting di v3)
...
w(SEP)
w("VWAP Mean-Reversion v4 — fixed time-based exit (ML8h recipe) — Validation Pipeline")
w(SEP)
...
def build_events(idx_range, z_entry, filter_variant, hmm_state, hold_hours):
    evs = []
    for i in idx_range:
        ...
        if filter_variant == "SIDEWAYS_ONLY":
            if hmm_state[i] != 1:   # 1 = sideways (see HMM_FEATURE_NAMES semantics)
                continue
        evs.append(dict(i=i, d=d, ep=CL[i], hold_hours=hold_hours))
    return evs


def run_bt(evs, slippage_pct=0.0):
    """v4: uscita a tempo fisso (hold_hours, cappato a fine giornata UTC),
    con un unico safety-stop largo e fisso (SAFETY_SL_ATR_MULT) — non più
    il tocco del VWAP come trigger primario."""
    if not evs:
    ...
    for ev in evs:
        ...
        hold = min(ev["hold_hours"], bars_to_dayend[i], N1H - 1 - i)
...
for tr_s, tr_e, oo_s, oo_e in WF_WINDOWS:
    ...
    for z_entry in Z_ENTRY_GRID:
        for filt in FILTER_VARIANTS:
            is_evs_base = build_events(idx_is, z_entry, filt, state_is_full, hold_hours=None)
            if len(is_evs_base) < 15:
                continue
            for hold_h in HOLD_HOURS_GRID:
                for e in is_evs_base:
                    e["hold_hours"] = hold_h
                is_res = run_bt(is_evs_base)
                sharpe = trade_level_sharpe(is_res["net_pnls"])
                if best is None or sharpe > best["sharpe"]:
                    best = dict(z_entry=z_entry, filt=filt, hold_h=hold_h,
                                sharpe=sharpe, n_is=len(is_evs_base))

    if best is None:
        continue
    selection_log.append(dict(window_start=tr_s, **best))
    oos_evs = build_events(idx_oos, best["z_entry"], best["filt"], state_oos_full,
                            hold_hours=best["hold_h"])
...
w(f"  hold_hours più scelto: {sel_df['hold_h'].value_counts().to_dict() if not sel_df.empty else {}}")
...
w(f"    Exit: safety-SL={res['n_sl']}  time={res['n_time']}")
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/vwap_mr_v4.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# VWAP Mean-Reversion v4 — fixed time-based exit (ML8h recipe) — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
