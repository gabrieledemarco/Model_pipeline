#!/usr/bin/env python3
"""
create_vwap_mr_v5_report.py
===============================
v5 — quinto tentativo di sfruttare l'edge trovato in v1 (win rate 61-66%
stabile ogni anno 2020-2026). Riepilogo dei tentativi precedenti:
  ...
  v4 uscita a tempo fisso (ricetta ML8h)               -> WR crolla a coin-flip: cambia la
                                                           natura stessa dell'edge (che è un
                                                           edge di LIVELLO — "il prezzo
                                                           ritocca il VWAP" — non di TIMING)

v1-v4 hanno tutti agito sullo STOP o sull'USCITA. v5 è la leva rimasta:
agire sul TARGET, mantenendo intatta la condizione di vittoria originale
(un livello VWAP-relativo, non un tempo fisso) ma allargandola oltre il
semplice tocco — per catturare l'overshoot che i processi di mean-reversion
spesso mostrano (il prezzo raramente si ferma esattamente alla media).

Layer 3 — Sizing/Exit (v5):
  TARGET: non più il tocco esatto di VWAP (Z=0), ma un livello Z-shiftato
    oltre il VWAP nella direzione del trade:
      target_price[j] = VWAP[j] + d × TARGET_OVERSHOOT × SIGMA[j]
    TARGET_OVERSHOOT ∈ {0.0, 0.5, 1.0} scansionato causalmente (0.0
    riproduce esattamente v1 come caso base/controllo di sanità)
  SL: fisso a 2.0×ATR_1H (v1, non scansionato — evita la trappola di
      overfitting di v3)
  Time-stop: fine giornata UTC (invariato)

Layer 1 (Entry) e Layer 2 (HMM Regime Gate) sono identici a v1: z-score
rispetto al VWAP di sessione (reset UTC 00:00, cumulativo, causale) con
soglia Z_ENTRY e filtro di regime SIDEWAYS_ONLY/NO_FILTER, entrambi
scansionati causalmente per finestra WFO insieme a TARGET_OVERSHOOT.

Selezione parametri: IS-scan causale per ogni finestra WFO (6m IS / 2m OOS
/ step 2m) su una griglia Z_ENTRY×filtro×TARGET_OVERSHOOT (3×2×3=18 combo),
selezione per Sharpe IS. Nessun DSR richiesto sul risultato finale: la
selezione avviene causalmente per finestra (dati IS, mai OOS).
...
TARGET_OVERSHOOT_GRID = [0.0, 0.5, 1.0]   # 0.0 = tocco esatto del VWAP (v1)
SL_ATR_MULT = 2.0                          # fisso (v1), non scansionato
...
w(SEP)
w("VWAP Mean-Reversion v5 — target allargato oltre il tocco VWAP — Validation Pipeline")
w(SEP)
...
def build_events(idx_range, z_entry, filter_variant, hmm_state, target_overshoot):
    evs = []
    for i in idx_range:
        ...
        if filter_variant == "SIDEWAYS_ONLY":
            if hmm_state[i] != 1:   # 1 = sideways (see HMM_FEATURE_NAMES semantics)
                continue
        evs.append(dict(i=i, d=d, ep=CL[i], target_overshoot=target_overshoot))
    return evs
...
def run_bt(evs, slippage_pct=0.0):
    ...
    for ev in evs:
        ...
        overshoot = ev["target_overshoot"]
        sl = ep - d * SL_ATR_MULT * ATR[i]
        hold = min(bars_to_dayend[i], N1H - 1 - i)
        out = "time"; exit_price = CL[min(i + hold, N1H - 1)]
        for k in range(1, hold + 1):
            j = i + k
            hk, lk, vwj, sigj = HI[j], LO[j], VWAP[j], SIGMA[j]
            if np.isnan(vwj) or np.isnan(sigj):
                continue
            target_j = vwj + d * overshoot * sigj
            if d == 1:
                if lk <= sl: out = "sl"; exit_price = sl; break
                if hk >= target_j >= lk:
                    out = "tp"; exit_price = target_j; break
            else:
...
for tr_s, tr_e, oo_s, oo_e in WF_WINDOWS:
    ...
    for z_entry in Z_ENTRY_GRID:
        for filt in FILTER_VARIANTS:
            for overshoot in TARGET_OVERSHOOT_GRID:
                is_evs = build_events(idx_is, z_entry, filt, state_is_full, overshoot)
                if len(is_evs) < 15:
                    continue
                is_res = run_bt(is_evs)
                sharpe = trade_level_sharpe(is_res["net_pnls"])
                if best is None or sharpe > best["sharpe"]:
                    best = dict(z_entry=z_entry, filt=filt, overshoot=overshoot,
                                sharpe=sharpe, n_is=len(is_evs))
...
w(f"  Overshoot più scelto: {sel_df['overshoot'].value_counts().to_dict() if not sel_df.empty else {}}")
...
w(f"\n{SEP}\n[DONE]\n{SEP}")
out_path = Path("reports/vwap_mr_v5.md")
out_path.parent.mkdir(exist_ok=True)
out_path.write_text("# VWAP Mean-Reversion v5 — target allargato oltre il tocco VWAP — Validation Pipeline\n\n```\n" +
                     "\n".join(report_lines) + "\n```\n", encoding="utf-8")
